...
class SendUnFollowUser @Inject constructor(private val repository: ProfileRepository) {
    data class Param(
        val userId: Long,
    )

    suspend operator fun invoke(param: Param): DomainResult<Unit, String> {
        when (val request = repository.requestUnFollowUser(profileId = param.userId)) {
            is DataResult.Fail -> {
