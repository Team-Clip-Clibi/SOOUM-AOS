...
class PagingNotificationUnReadTest {
    ...
    private lateinit var pagingSource: PagingNotificationUnRead

    @Before
    fun setUp(){
        pagingSource = PagingNotificationUnRead(
    ...
    @Test
    fun refreshLoad_withVailData_returnsCorrectPage() = runTest {
        ...
        coEvery { networkRepository.requestNotificationUnRead(any()) } returns successResult
    ...
    fun refreshLoad_whenNetworkResultFails_returnError() = runTest {
        ...
        coEvery { deviceRepository.requestToken(any()) } returns fakeToken
        coEvery { networkRepository.requestNotificationUnRead(any()) } returns failResult
    ...
    fun appendLoad_whenKeyIsValid_loadsNextPageSuccessfully() = runTest {
        ...
        coEvery { deviceRepository.requestToken(any()) } returns fakeToken
        coEvery {
            networkRepository.requestNotificationUnReadPatch(
                accessToken = any(),
                lastId = previousKey
            )
